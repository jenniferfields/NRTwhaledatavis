...
Monthwhaleent
ggsave(filename = "Output/MonthWhalesSCCppt.jpg", device = "jpeg", width = 30, height = 20)
...
whalessum<-shipyearspp%>%
  group_by(Common.Name,Year)%>%
  summarise(sum=sum(n)) %>%
  group_by(Common.Name)%>%
  summarise(total = sum(sum))
write.csv(whalessum, file= 'output/vesselstrikesbyspp.csv')

#Unknown2, Minke 2, Sei 2 Sperm 1, Fin 17, Gray 30, Blue 7, Humpback 18
#gray 16/30 in 2018/2019
...
summarywhaleandstate<-st.sp.countycomb%>%
  ...
  dplyr::summarise(total=sum(n)) 
write.csv(summarywhaleandstate, file= 'output/vessilestrikesbystate.csv')
#Gray CA - 22/79 27.8%
#humpback CA 15/79 19.0%
#fin CA 14/79 17.7%
#sei & unknown 2.5%, minke 1.27%, 0% sperm
...
Monthwhalest
ggsave(filename = "Output/MonthshipSCCppt.jpg", device = "jpeg", width = 30, height = 20)
